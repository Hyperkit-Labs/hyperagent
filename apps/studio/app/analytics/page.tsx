  ...
  return (
    <div className="glass-panel p-5 rounded-xl">
      ...
      <div className="flex items-center gap-2 mt-2">
        ...
        {trend && (
          <span
            className={`text-xs font-medium ${trend.startsWith("↑") ? "text-[var(--color-semantic-success)]" : trend.startsWith("↓") ? "text-[var(--color-semantic-error)]" : "text-[var(--color-text-muted)]"}`}
          >
            {trend}
          </span>
        )}
      ...
      <div className="max-w-[1200px] mx-auto space-y-6 animate-enter">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard
            ...
            value={loading ? "..." : total}
            icon={Activity}
            color="text-[var(--color-semantic-violet)]"
            trend={total > 0 ? "↑ vs last period" : undefined}
            loading={loading}
          />
          <MetricCard
            label="Active"
            ...
            icon={Loader2}
            color="text-[var(--color-semantic-info)]"
            loading={loading}
          />
          <MetricCard
            label="Completed"
            ...
            icon={CheckCircle}
            color="text-[var(--color-semantic-success)]"
            trend={completed > 0 ? "↑" : undefined}
            loading={loading}
          />
          <MetricCard
            label="Failed"
            ...
            icon={XCircle}
            color="text-[var(--color-semantic-error)]"
            trend={failed > 0 ? "↓" : undefined}
            loading={loading}
          />
        </div>
            ...
            <div className="glass-panel rounded-xl p-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-lg font-semibold ${successRate >= 80 ? "text-green-400" : successRate >= 50 ? "text-amber-400" : "text-red-400"}`}
                  >
                    {successRate}%
                  </span>
                </div>
        ...
        {view === "network" && (
          <div className="glass-panel rounded-xl p-6">
            ...
            <p className="text-xs text-[var(--color-text-muted)] mt-4">
              Multi-chain analytics will be available when additional networks
              are enabled.
            </p>
          </div>
        )}
        ...
        {view === "security" && (
          <div className="glass-panel rounded-xl p-6">
            ...
            <p className="text-xs text-[var(--color-text-muted)] mt-4">
              Audits use Slither and Mythril for static analysis. Tenderly
              simulations validate behavior before deployment.
            </p>
          </div>
        )}
      </div>
