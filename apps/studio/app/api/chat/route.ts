...
const JWT_SECRET =
  process.env.AUTH_JWT_SECRET || process.env.SUPABASE_JWT_SECRET || "";
...
function resolveModel(req: Request): ResolvedModel | Response | null {
  ...
  if (hasByokIntent) {
    ...
    return {
      model: buildModel(provider, rawKey),
      provider,
  ...
  if (!isProduction) {
    for (const [envVar, provider] of [
      ...
      if (key?.trim()) {
        ...
        return {
          model: buildModel(provider, key.trim()),
          provider,
          label: PROVIDER_LABELS[provider],
        };
      }
...
async function callBackendWorkflow(
  ...
  const res = await fetch(`${base}/workflows/generate`, {
    ...
    body: JSON.stringify({
      ...
      ...(body.api_keys && Object.keys(body.api_keys).length > 0
  ...
  if (!res.ok) {
    ...
    if (res.status === 402) {
      msg = msg.trim();
      if (
        !msg.toLowerCase().includes("payments") &&
...
export async function POST(req: Request) {
  ...
  if (!guardrail.passed) {
    return new Response(
      JSON.stringify({
        error: guardrail.violation ?? "Security policy violation",
      }),
      { status: 400, headers: { "Content-Type": "application/json" } },
    );
  }
  ...
  if (resolved) {
    ...
    try {
            ...
            parameters: jsonSchema<{
              ...
              properties: {
                prompt: {
                  type: "string",
                  description:
                    "Natural language description of the smart contract to create.",
                },
                network: {
                  type: "string",
                  description:
                    "Target blockchain network (e.g. skalebase-sepolia).",
                },
                template_id: {
                  type: "string",
      ...
      return new Response(
        JSON.stringify({
          error: "LLM request failed. Check your API key and quota.",
        }),
