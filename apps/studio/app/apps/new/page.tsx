...
export default function NewAppPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setWorkflowId(null);
    try {
      const { ok } = await requireLLMKeys(activeWorkspaceId !== "default" ? activeWorkspaceId : undefined);
      if (!ok) {
      ...
      const { workflow_id } = await createWorkflow(
        ...
        { headers: workspaceHeaders(activeWorkspaceId !== "default" ? activeWorkspaceId : undefined) }
      );
          ...
          <div className="max-w-[800px] mx-auto space-y-6 animate-enter">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-xl font-semibold text-[var(--color-text-primary)] tracking-tight">Building</h1>
                  <p className="text-sm text-[var(--color-text-tertiary)]">Agent is building your DApp base layer.</p>
                </div>
              </div>
              <Link
            ...
            <div className="rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-panel)] overflow-hidden">
              {discussionEvents.length > 0 ? (
                <XTermTerminal events={discussionEvents} className="min-h-[280px]" />
              ) : (
