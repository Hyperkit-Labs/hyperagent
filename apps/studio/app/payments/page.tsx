...
function PaymentsHeroMetric({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-[var(--color-border-subtle)] glass-panel px-4 py-3">
      <div
        className={cn("absolute inset-0 opacity-30 bg-gradient-to-br", accent)}
      />
      <div className="relative flex flex-col gap-1">
        ...
        <span className="text-xl font-semibold tracking-tight text-[var(--color-text-primary)]">
          {loading ? (
            "..."
          ) : numericValue != null && !Number.isNaN(numericValue) && suffix ? (
            <>
...
export default function PaymentsPage() {
  ...
  const balanceValue = balance
    ...
    : loading
      ? "..."
      : "0 USD";
  const spendValue = loading
    ? "..."
    : `${summary.total ?? "0"} ${summary.currency ?? "USD"}`;
  const budgetValue = loading
    ? "..."
    : `${control?.budget ?? "0"} ${control?.currency ?? "USD"}`;
        ...
        <div className="max-w-[1200px] mx-auto space-y-6 animate-enter">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <PaymentsHeroMetric
              ...
              numericValue={
                typeof summary.total === "number"
                  ? summary.total
                  : parseFloat(String(summary.total ?? 0)) || undefined
              }
            ...
            <PaymentsHeroMetric
              ...
              tone="warning"
              numericValue={
                typeof control?.budget === "number"
                  ? control.budget
                  : parseFloat(String(control?.budget ?? 0)) || undefined
              }
              suffix={control?.currency ?? "USD"}
              loading={loading}
            />
          </div>
