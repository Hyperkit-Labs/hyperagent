...
export function WorkflowStages({
  ...
  const stageMap = new Map(
    backendStages.map(
      (s: { name?: string; stage?: string; status?: string }) => [
        s.name ?? s.stage ?? "",
        s,
      ],
    ),
  ...
  const errorSource = (workflow.metadata ?? workflow.meta_data) as
  ...
  const steps: PlanStep[] = activeStageDefs.map((stageDef) => {
    const stage = stageMap.get(stageDef.name) as
      | { status?: string; error?: string; cycles?: number }
      | undefined;
    let status: PlanStepStatus = stage
    ...
    if (
      ...
      const prevDone =
        idx > 0 &&
        activeStageDefs.slice(0, idx).every((prev) => {
    ...
    if (stageDef.name === "codegen" && !stage && contractData?.bytecode)
      status = "completed";
    if (stageDef.name === "deploy" && !stage) {
      const deployResult = (
        workflow as { deployment_result?: { contract_address?: string } }
      ).deployment_result;
    ...
    const baseIcon = STAGE_ICONS[stageDef.name] ?? (
      <Clock className="w-5 h-5" />
    );
    const icon =
      stageDef.name === "audit" && status === "processing" ? (
        <span className="inline-flex animate-radar" aria-hidden>
          {baseIcon}
        </span>
      ) : (
        baseIcon
    ...
    <div className="space-y-3">
      ...
      {workflowFailed && !workflowError && (
        <div className="rounded-lg border border-red-500/20 bg-red-500/5 px-3 py-2">
          <p className="text-xs font-medium text-red-400">
            Pipeline failed. Check logs or retry with a different prompt.
          </p>
        </div>
      )}
      {hasUiSchema && workflowId && (
        <div className="flex items-center gap-2">
          ...
          {exportError && (
            <span className="text-xs text-[var(--color-semantic-error)]">
              {exportError}
            </span>
          )}
        </div>
