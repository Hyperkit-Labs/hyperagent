...
function getContentAndScrollButton(
  children: React.ReactNode
): { content: ReactNode; hasScrollButton: boolean } {
  let content: ReactNode = null;
  let hasScrollButton = false;
  Children.forEach(children, (child) => {
    if (child && typeof child === 'object' && 'type' in child && (child as { type: unknown }).type === ConversationContent) {
      content = (child as React.ReactElement<{ children?: ReactNode }>).props.children;
    } else if (child && typeof child === 'object' && 'type' in child && (child as { type: unknown }).type === ConversationScrollButton) {
...
export function Conversation({
  ...
  const scrollButtonElement = hasScrollButton
    ? Children.map(children, (child) =>
        child && typeof child === 'object' && 'type' in child && (child as { type: unknown }).type === ConversationScrollButton
          ? cloneElement(child as React.ReactElement<{ onClick?: () => void }>, { onClick: scrollToBottom })
          : null
      )?.find(Boolean)
    : null;
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-6">
        {isEmpty && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <p className="text-[var(--color-text-tertiary)] text-sm">{emptyMessage}</p>
          </div>
        )}
        {content}
      </div>
      <div className="absolute right-6 bottom-4 flex items-center gap-2">
        {showScrollButton && (scrollButtonElement ?? <ConversationScrollButton onClick={scrollToBottom} />)}
