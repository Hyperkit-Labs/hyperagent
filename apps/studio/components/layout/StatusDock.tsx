      ...
      >
          {runStatus === "running" ? (
            <>
              ...
              <span>{activeRunName ? `${activeRunName.slice(0, 24)}...` : "VM running"}</span>
              <span className="text-slate-500">Live logs</span>
            </>
          ) : (
            <>
              <span className={`h-2 w-2 rounded-full ${runStatus === "failed" ? "bg-red-400" : "bg-slate-500"}`} />
              <span>{runStatus === "failed" ? "Pipeline failed" : "Idle"}</span>
