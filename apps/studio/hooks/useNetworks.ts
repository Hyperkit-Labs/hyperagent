              ...
              metrics && typeof metrics === "object"
                ? data.map((network: NetworkConfig) => {
                    ...
                    const networkMetrics =
                      networksMap?.[network.network_id ?? network.id] || {};
                    return {
                      ...
                      status: (networkMetrics.status === "degraded" ||
                      networkMetrics.status === "offline"
                        ? networkMetrics.status
                        : "online") as NetworkStats["status"],
                    };
                  })
                : data.map((network: NetworkConfig) => ({
                    chainId: network.chain_id ?? 0,
