...
export function appendRequestIdHint(message: string, requestId?: string | null): string {
...
export function getErrorRequestId(err: unknown): string | undefined {
  ...
  return typeof r.requestId === 'string' && r.requestId.trim() ? r.requestId.trim() : undefined;
}
...
export function bootstrapConfigFailureMessage(
  ...
  } else if (status === 408 || (rawMessage && /abort|timed out/i.test(rawMessage))) {
    base =
  ...
  } else if (rawMessage && rawMessage.length < 200 && !looksLikeRawHttp(rawMessage)) {
...
function looksLikeRawHttp(s: string): boolean {
  ...
  return lower.includes('internal server') || lower.includes('unauthorized') || lower.startsWith('http ');
}
...
export function signInFailureMessage(
  ...
  if (code) {
    ...
    if (mapped) {
      const withRoute = mapped.includes('return route') ? mapped : `${mapped.replace(/\s*$/, '')}${RETURN_ROUTE_HINT}`;
...
export function workflowCreateFailureMessage(err: unknown): string {
  ...
  if (status === 401 || status === 403) {
    ...
    base = 'Not enough credits to start a run. Add budget in Payments, then try again. Your draft is still here.';
  } else if (status === 408) {
    base = 'Starting the workflow timed out. Your draft is still here. Try again.';
  } else if (status === 429) {
    base = 'Too many requests right now. Wait a moment and try again. Your draft is still here.';
  } else if (status != null && status >= 500) {
    base = "We couldn't create the workflow right now. Your draft is still here. Try again in a moment.";
  } else {
    base = "We couldn't create the workflow. Your draft is still here. Check the highlighted issues and try again.";
  }
  return appendRequestIdHint(base, requestId);
}
