...
| **Permission** | Explicit allow / ask / deny (and escalation) for tools, files, network | Studio session and wallet gates: `apps/studio/middleware.ts`, `apps/studio/components/auth/`, `apps/api-gateway/src/auth.ts`; product policy in [permission-approval-policy.md](permission-approval-policy.md) |
