...
- **Default network:** Frontend uses the first testnet from the API (`is_mainnet === false`) or the fallback slug from the registry (e.g. `skalebase-sepolia`). Sepolia is the default network. No env var; single source is the chain registry.
