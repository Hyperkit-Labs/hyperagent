...
**Fix:** Set both `SUPABASE_URL` and `SUPABASE_SERVICE_KEY` on the orchestrator service (e.g. `api-gateway` or `orchestrator` in Contabo/Coolify). See [Deployment Runbook](runbooks/deployment-runbook.md).
