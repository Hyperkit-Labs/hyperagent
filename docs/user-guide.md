...
1. **Connect** – Connect your wallet in the web app (e.g. MetaMask, WalletConnect). This identifies you and can be used for deployment and payments.
2. **BYOK (Bring your own keys)** – Add your LLM API keys (e.g. OpenAI, Anthropic, Google, OpenRouter) in the app. Keys are stored in an isolated, encrypted environment and used only for your workflows. The platform does not use server-side LLM keys for your runs.
3. **Approved** – After keys are set and validated, you can run workflows.
4. **Run** – Start a workflow: describe your contract, then the pipeline runs spec, generation, audit, Tenderly simulation and report, and (if you choose) deployment. You see simulation and report in the same flow.
...
- **Workflows** – Create and monitor runs. View status, logs, and results.
- **Contracts** – View generated Solidity and ABIs.
- **Deployments** – See deployment records and explorer links when you deploy.
- **Templates** – Browse and use contract templates when creating workflows.

---

## Security and simulation

- Generated code is audited with static analysis (e.g. Slither) and simulated (e.g. Tenderly) before deployment.
