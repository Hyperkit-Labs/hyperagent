"""Test generation agent: call agent-runtime /agents/test. BYOK via X-Agent-Session when JWT present."""

import os

import httpx
from registries import get_timeout

from agents.agent_http import agent_runtime_headers

AGENT_RUNTIME_URL = os.environ.get("AGENT_RUNTIME_URL", "http://localhost:4001")


async def generate_tests(
    contracts: dict,
    spec: dict,
    design: dict,
    user_id: str,
    project_id: str,
    run_id: str,
    api_keys: dict,
    agent_session_jwt: str | None = None,
) -> dict:
    """Call agent-runtime POST /agents/test to generate Foundry/Hardhat tests."""
    context: dict = {
        "userId": user_id,
        "projectId": project_id,
        "runId": run_id,
        "apiKeys": api_keys or {},
    }
    headers = agent_runtime_headers(agent_session_jwt)
    body: dict = {
        "contracts": contracts,
        "spec": spec,
        "design": design,
        "context": context,
    }
    async with httpx.AsyncClient(timeout=get_timeout("codegen")) as client:
        r = await client.post(
            f"{AGENT_RUNTIME_URL.rstrip('/')}/agents/test",
            json=body,
            headers=headers,
        )
        r.raise_for_status()
        return r.json()
