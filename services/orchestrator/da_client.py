...
EIGENDA_ENABLED = (os.environ.get("EIGENDA_ENABLED") or "").strip().lower() in ("1", "true", "yes")
EIGENDA_PRIVATE_KEY = (os.environ.get("EIGENDA_PRIVATE_KEY") or "").strip()
EIGENDA_DISPERSER_URL = (os.environ.get("EIGENDA_DISPERSER_URL") or "https://disperser-holesky.eigenda.xyz").rstrip("/")
...
def _extract_da_proof(result: object | None, blob_id: str) -> tuple[str, str]:
    ...
    if result and hasattr(result, "blob_header"):
        ...
        da_cert = getattr(header, "commitment", None) or getattr(header, "blob_verification_proof", None)
        ref_block = getattr(header, "reference_block_number", None) or getattr(header, "batch_header", None)
        if hasattr(result, "batch_header") and ref_block is None:
            ref_block = getattr(result.batch_header, "reference_block_number", None) or str(getattr(result, "batch_id", ""))
    if hasattr(result, "blob_verification_proof") and da_cert is None:
        da_cert = str(result.blob_verification_proof) if result.blob_verification_proof else None
...
async def submit_blob(blob_id: str, payload_json: str) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        disperse_fn = getattr(client, "disperse_blob", None) or getattr(client, "disperse", None)
        if not disperse_fn:
            raise RuntimeError("EigenDA configured but client has no disperse_blob or disperse method")
        ...
        logger.info("[da] EigenDA blob submitted blob_id=%s da_cert=%s ref_block=%s", blob_id, da_cert[:32] if da_cert else "", ref_block)
        return da_cert, ref_block
    except ImportError as e:
        raise RuntimeError(f"EigenDA enabled but powerloom-eigenda not installed: {e}") from e
    except RuntimeError:
        raise
