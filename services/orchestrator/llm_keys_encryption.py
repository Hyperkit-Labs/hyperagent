...
try:
    from llm_keys_kms import _is_envelope_blob, _is_kms_configured, decrypt_llm_keys_kms, encrypt_llm_keys_kms
except ImportError:
...
def _get_fernet_key() -> bytes:
    ...
    if not raw or len(raw) < 44:
        raise ValueError("LLM_KEY_ENCRYPTION_KEY must be set (44-char base64url from Fernet.generate_key())")
